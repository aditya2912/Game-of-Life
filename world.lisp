...
;; Create the main sdl window and context, and init opengl;; Once our graphics are set up, just to the main loop (world-run) 
(defmethod world-init ((w world))
  ...
  (let ((vport (coerce (gl:get-integer :viewport) 'list)))
    ...
    (gl:push-matrix)
    (gl:load-identity)
    ;; this renders the actual 2d
    (gl:ortho  0 (nth 2 vport) 0 (nth 3 vport) -1 1)
    (gl:matrix-mode :modelview)
    (gl:push-matrix)
    (gl:load-identity))
  ;; disable depth testing (not neede in 2d) 
  (gl:disable :depth-test)
  (gl:clear-color .04 .04 .04 0)
  ;; run the world , this calls our actual game loop
  (world-run w)))

;; main gol "Algo".. if a slot is switched on (alive), it only stays alive if it has either 2 or 3 neighbours. If a slot is empty, it can only be turned on if it has exactly 3 neighbours.

(defparameter *gol-algorithm-original*
  (lambda (life)
    ;; create an initial lookup table (populated below)
    ;; and a next-gen container for storing all life who made it past this gen.
    ;; we also init "slots-to-check" which will be populated with actual slots we want to iterate over check calculating the next generation (as opposed to looping over the entire grid slot by slot)
    (let ((grid-lookup (make-hash-table :size (length life)))
      ...
      (labels ((get-xy-hash-key (x y) (intern (format nil "~dx~d" x y)))
		 ...
		 (let ((surrounding (get-surrounding-slots x y))
		       (life-count 0))
		   ;; loop over our surrounding coordinates list and check each point for life, increasing life count if its found
		   (dolist (pair surrounding)
			 ...
			 (incf life-count))))
		   life-count)))
	;; populate our empty hash lookup such that active life forms can be looked up by specific x,y coordinates
	(dolist (l life)
	  (let ((x (nth 0 l))
		(y (nth 1 l)))
	    ;; loop over slots that may be affected and add them to our makeshift set
      ...
      ;; loop over slots that may possibly be affected during processing
      (loop for pair being the hash-keys in slots-to-check using (hash-value v) do
	   (let ((x (nth 0 pair))
	     ...
	     (when (let ((life-here (life-exists-here x y grid-lookup))
			 ...
			 (if (eql surrounding-count 3) tnil)))
	       (push `(,x ,y) next-gen)))))
      next-gen)))
...
;; Add a life at a given grid point
...
;; There are no carbon based life forms in this sector(not calling this)
...
;; Step the world. runs the physics simulation and processes any pre.post sim callbacks
...
;; runs the world .. This is the main game loop  and includes event handling
...
;; process life ... decide who lives and dies based on gol rules.
;; Takes a lambda which provides the actual algorithm to use
